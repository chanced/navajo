...
use crate::{
    gen_id, id::gen_unique_id, rand, timestamp, DecryptError, EncryptError, KeyInfo,
...
use self::{header::Header, salt::Salt};
...
impl Aead {
    ...
    pub fn encrypt<A>(&self, cleartext: Bytes, additional_data: A) -> Result<Vec<u8>, EncryptError>
    where
        A: AsRef<[u8]>,
    {
    ...
    fn key_by_id(&self, id: u32) -> Option<Arc<Key>> {
...
impl Key {
    ...
    fn decrypt(&self, mut buf: BytesMut, additional_data: &[u8]) -> Result<Vec<u8>, DecryptError> {
        ...
        let nonce = ring::aead::Nonce::try_assume_unique_for_key(
            &buf.split_to(self.algorithm.nonce_len()),
        )?;
        let aad = Aad::from(additional_data);
...
mod tests {
    ...
    fn test_encrypt_produces_correct_header() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(buf.get_u8(), Method::Online);
        assert_eq!(buf.get_u32(), ks.primary_key().unwrap().id);
        assert_eq!(
            buf.remaining(),
    ...
    fn test_round_trip() -> Result<(), Box<dyn std::error::Error>> {
        ...
        buf.advance(k.algorithm.nonce_len());
        let encrypted = buf.split_to(cleartext.len());
